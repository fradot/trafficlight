...
/**
 * This class represents a Traffic Light configuration. It defines how long each state should be
 * displayed and which priority this configuration has over other overlapping configurations.
 * <code>startCronExpression</code>
 * is used to define the periodic start time of this configuration
 * <code>endCronExpression</code>
 * defines the periodic end time of this configuration
 * <code>priority</code>
 * the configuration priority defines the behaviour of the application when two or more
 * configurations are active at the same time. In this case, the configuration with highest priority
 * will be used.
 * <code>defaultConfiguration</code>
 * A Default configuration is always active and has the lowest priority.
 */
@Entity
@Table(name = TrafficLightConfiguration.TABLE)
public class TrafficLightConfiguration
    implements Serializable, Comparable<TrafficLightConfiguration> {

  public static final String TABLE = "tl_configuration";
  private static final AtomicLong seq = new AtomicLong(0);

  @Transient private Long thisSeq;
  ...
  public int compareTo(TrafficLightConfiguration other) {
    ...
    if (other != null && !other.equals(this)) {
      if (other.getPriority() > this.priority) return -1;
      else if (other.getPriority() < this.priority) return 1;
      else if (other.getPriority().equals(this.priority))
        return other.getSeq() > this.thisSeq ? -1 : 1;
    } else if (other != null && other.equals(this)) return 0;
