...
/**
 * This class represents a traffic light task and is meant to enable or disable a {@link TrafficLightConfiguration}.
 */
public class TrafficLightRunnableTask implements Runnable {
    ...
    public TrafficLightRunnableTask(PriorityBlockingQueue<TrafficLightConfiguration> trafficLightConfigurationQueue,
                                    TrafficLightConfiguration trafficLightConfiguration, Boolean enabling, Boolean disabling) {
        ...
        if(enabling && disabling) {
            throw new IllegalArgumentException("A Traffic Light task can't be an enabling and disabling task at the same time");
        }
    }
    ...
    public void run() {
        ...
        if (trafficLightConfigurationQueue.contains(this.trafficLightConfiguration) &&
                !this.trafficLightConfiguration.isDefaultConfiguration() && this.disabling) {
